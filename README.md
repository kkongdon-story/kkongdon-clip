...
### 📹 유튜브 캡처
- **`Ctrl+Shift+S`** — 자막 전체를 한 번에 저장
- **`Ctrl+Shift+F`** — 현재 프레임 이미지 첨부
- 다국어 자막 자동 선택 · STT(Whisper) 폴백 (자막 없는 영상도 OK)
- AI 자막 정리 — 중복 제거, 문장 자연스럽게 다듬기
- 영어 → 한국어 자동 번역 (또는 원하는 언어)

### 🌐 웹 페이지 캡처
- **`Ctrl+Shift+A`** — 현재 페이지 본문 전체 저장
- 광고·메뉴 제거, 본문만 깔끔하게 추출
- 이미지 URL 포함 옵션 (레시피, 제품 리뷰 등)
- 선택 텍스트만 저장하는 하이라이트 캡처 지원
